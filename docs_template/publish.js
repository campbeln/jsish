...
function buildMemberNav(items, itemHeading, itemsSeen, linktoFn) {
    var nav = '';
    var bMixin = (itemHeading.trim().toLowerCase() === "mixins"); // neek
    ...
    if (items && items.length) {
        ...
        items.forEach(function(item) {
            ...
            if (item.tags && item.tags.length > 0) {
                item.tags.forEach(function (tag) {
                    sText = tag.text.trim().toLowerCase();
                    item.customTags[tag.title] = (
                        sText === "true" ? true : (
                            sText === "false" ? false :
                                tag.text || true
                        )
            ...
            if (bMixin) {
                item.customTags.$frommixin = true;
            }
            // /neek
            ...
            if ( !hasOwnProp.call(item, 'longname') ) {
                ...
                itemsNav += linktoFn(item.longname,
                    displayName.replace(/\b(module|event):/g, '').replace(/!/g, "").replace(/:/g, ".") +
                    (bMixin ? " <mixin title='From Mixin: " + item.name + ".js'>[M]</mixin>" : "") +
                    (item.customTags.$clientsideonly ? " <client title='Client-side only'>[C]</client>" : "") +
                    (item.customTags.$serversideonly ? " <server title='Server-side only'>[S]</server>" : "")
                ...
                if (methods.length) {
                    ...
                    methods.forEach(function (method) {
                        ...
                        if (!bMixin && method.meta.shortpath.toLowerCase() !== "ish.js") {
                            method.customTags.$frommixin = true;
                        }
                        method.item = item;
                        // /neek
        ...
        if (itemsNav !== '') {
            nav += '<h3>' + (bMixin ? "Namespace " : "") + itemHeading + '</h3><ul>' + itemsNav + '</ul>';
        }
    }
