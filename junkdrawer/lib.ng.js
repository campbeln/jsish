    ...
    core.lib.ng = {
        ...
        scope: core.extend(
            function (vDom) {
                ...
                is: function ($scope) {
                    return $scope instanceof core.lib.ng.$rootScope.constructor;
                }
            }
        ), //# core.lib.ng.scope
         ...
         * @param {varient} vDom - Varient representing the DOM object.
         * @param {varient} [vScope] - Optional varient representing the Angular scope or DOM object under the required Angular scope.
         * @returns {object} - DOM object managed by Angular.
        ################################################################################################# */
        compile: function (vDom, vScope) {
            var _dom = core.type.dom.mk(vDom, null),
                $scope = (core.lib.ng.scope.is(vScope) ? vScope : core.lib.ng.scope(vScope || vDom)),
                $returnVal /* = undefined*/
            ;
        ...
        init: function (oOptions) {
            ...
            ngApp
                ...
                .run(['$filter', '$http', '$timeout', '$compile', '$rootScope', function ($filter, $http, $timeout, $compile, $rootScope) {
                    ...
                    //# Set the $rootScope outside of .extend to avoid too much recursion
                    //#     TODO: Why does this cause too much recursion error in .extend!?!?
                    core.lib.ng.$rootScope = $rootScope;

                    //# .register $timeout and $compile with .lib.ui then .call our .onload
                    core.lib.ui.sync.register($timeout);
                    core.lib.ui.bind.register(function (_dom, $scope) {
                        return $compile(_dom)($scope);
                    });
